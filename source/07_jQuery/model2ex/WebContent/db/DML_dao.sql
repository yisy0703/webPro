...
DELETE FROM FILEBOARD WHERE FGROUP = 127 AND (FSTEP>=0 AND 
    FSTEP<(SELECT NVL(MIN(FSTEP),9999) 
          FROM FILEBOARD WHERE FGROUP=127 AND FSTEP>0 AND FINDENT<=0));
UPDATE FILEBOARD SET fSTEP = fSTEP-2 WHERE fGROUP=2 AND fSTEP>1;-- 생략 가능(2은 위의 DELETE문 수행결과)
COMMIT;
