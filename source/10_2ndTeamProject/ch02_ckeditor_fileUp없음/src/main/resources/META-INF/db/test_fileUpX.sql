...
SELECT * FROM (SELECT ROWNUM RN, A.* FROM 
      (SELECT * FROM B WHERE UPPER(BTITLE) LIKE '%'||UPPER('테스트')||'%' ORDER BY BNO DESC) A)
  WHERE RN BETWEEN 1 AND 11;
  -- ③ schItem이 'bcontent'일 때
SELECT * FROM (SELECT ROWNUM RN, A.* FROM 
      (SELECT * FROM B WHERE UPPER(BCONTENT) LIKE '%'||UPPER('내')||'%' ORDER BY BNO DESC) A)
  WHERE RN BETWEEN 1 AND 11;  
  -- ④ schItem이 'btitleBcontent'일 때
SELECT * FROM (SELECT ROWNUM RN, A.* FROM 
      (SELECT * FROM B WHERE UPPER(BTITLE) LIKE '%'||UPPER('t')||'%' OR UPPER(BCONTENT) LIKE '%'||UPPER('t')||'%'  ORDER BY BNO DESC) A)
  WHERE RN BETWEEN 1 AND 11;
...
SELECT COUNT(*) FROM B;
  -- ② schItem이 'btitle'일 때
SELECT COUNT(*) FROM B WHERE UPPER(BTITLE) LIKE '%'||UPPER('t')||'%';
  -- ③ schItem이 'bcontent'일 때
SELECT COUNT(*) FROM B WHERE UPPER(BCONTENT) LIKE '%'||UPPER('t')||'%' ;
  -- ④ schItem이 'btitleBcontent'일 때
SELECT COUNT(*) FROM B WHERE UPPER(BTITLE) LIKE '%'||UPPER('t')||'%' OR UPPER(BCONTENT) LIKE '%'||UPPER('t')||'%';
